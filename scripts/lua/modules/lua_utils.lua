...
function flowStatusToMessage(status)
   -- NOTE: keep in sync with Flow::flowStatus2Str
   local status_to_msg = {
      normal                  = "Normal",
      slow_tcp_connection     = "Slow TCP Connection",
      slow_app_header         = "Slow Application Header",
      slow_data               = "Slow Data Exchange (Slowloris?)",
      low_goodput             = "Low Goodput",
      syn_probing             = "Suspicious TCP SYN Probing (or server port down)",
      tcp_connection_issues   = "TCP Connection Issues",
      tcp_probing             = "Suspicious TCP Probing",
      alerted_interface       = "Flow emitted during alerted interface",
      tcp_connection_refused  = "TCP connection refused",
      unknown_status          = "Unknown status",
   }

   if status_to_msg[status] ~= nil then
...
function formatFlowStatus(status)
   ...
   return "<font color="..color..">"..flowStatusToMessage(status).."</font>"
end
