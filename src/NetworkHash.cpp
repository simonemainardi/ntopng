...
NetworkHash::NetworkHash(NetworkInterface *_iface, u_int _num_hashes, u_int _max_hash_size) 
...
Network* NetworkHash::get(u_int16_t vlanId, int16_t networkId) {
  ...
  if(table[hash] == NULL) {
    ...
    head = (Network*)table[hash];
    
    while(head != NULL) {      
      if((!head->idle())
	 && (head->get_vlan_id() == vlanId)
	 && (head->get_network_id() == networkId))
	break;
      else
	head = (Network*)head->next();
    }
