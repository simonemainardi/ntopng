...
void NetworkInterface::processFlow(ZMQ_Flow *zflow) {
  ...
  /* Update Mac stats */
  if(srcMac) {
    srcMac->incSentStats(zflow->pkt_sampling_rate * (src2dst_direction ? zflow->out_pkts : zflow->in_pkts),
			 zflow->pkt_sampling_rate * (src2dst_direction ? zflow->out_bytes : zflow->in_bytes));
    srcMac->incRcvdStats(zflow->pkt_sampling_rate * (src2dst_direction ? zflow->in_pkts : zflow->out_pkts),
  ...
  if(dstMac) {
    dstMac->incSentStats(zflow->pkt_sampling_rate * (src2dst_direction ? zflow->in_pkts : zflow->out_pkts),
			 zflow->pkt_sampling_rate * (src2dst_direction ? zflow->in_bytes : zflow->out_bytes));
    dstMac->incRcvdStats(zflow->pkt_sampling_rate * (src2dst_direction ? zflow->out_pkts : zflow->in_pkts),
			 zflow->pkt_sampling_rate * (src2dst_direction ? zflow->out_bytes : zflow->in_bytes));
  }
